...
function renderCart(cart) {
  ...
  filteredCart.forEach((item, idx) => {
    ...
    const sizeView = item.size
      ? `${item.size}${item.sizeExtra ? ` (+${toTHB(item.sizeExtra)})` : ""}`
      : "";
...
function wireBackButton() {
  ...
  backBtn.addEventListener("click", () => {
    if (CONFIG.ROUTES.HOME) location.href = CONFIG.ROUTES.HOME;
    else history.back();
  });
}
...
function wireQtyActions() {
  ...
  list.addEventListener("click", async (e) => {
    ...
    if (action === "plus") cart[idx].qty = (cart[idx].qty || 0) + 1;
...
async function submitOrder(cart, grandTotal) {
  ...
  return { status: Math.random() < 0.85 ? "success" : "fail" };
}
...
function wireConfirmButton(cartProvider) {
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      } else location.href = CONFIG.ROUTES.FAIL;
    } catch (err) {
      console.error(err);
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const getCart = async () => {
    try {
      // โหลด cart object จาก simple_cart_v1
      const cartObj = JSON.parse(localStorage.getItem("simple_cart_v1")) || {};

      // แปลงเป็น array ของ object สำหรับ renderCart
      const cartArray = Object.keys(cartObj).map((id) => {
        if (id === "1") return { id: 1, name: "ข้าวสวย", price: 10, qty: cartObj[id], image: "img/rice.jpg" };
        // เมนูอื่น ๆ จาก pending_add
        const pending = JSON.parse(localStorage.getItem("pending_add") || "{}");
        if (pending.id === id) {
          return { id, name: pending.name, price: Number(pending.priceText.replace(/[^\d]/g, "")) || 0, qty: cartObj[id], image: pending.image || "img/placeholder.webp" };
        }
        return null;
      }).filter(Boolean);
